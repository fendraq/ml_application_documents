...
print("Training KNC")
knc.fit(X_train_pca, y_train) # without pca: X_trained_scaled

print("Training RFC")
rfc.fit(X_train_pca, y_train) # without pca: X_trained_scaled

print("Setting threshold to 0.4")
threshold = 0.4
rfc_y_proba = rfc.predict_proba(X_test_pca)[:, 1] # without pca: X_test_scaled
rfc_y_pred = (rfc_y_proba >= threshold).astype(int)

knc_y_proba = knc.predict_proba(X_test_pca)[:, 1] # without pca: X_test_scaled
...
print("Scaler saved")
joblib.dump(pca, "../models/pln_first_page_model_128x128/pca_model.pkl")
print("PCA saved")